...
describe('Barnetilsyn', () => {
  ...
  describe('2 barn, samme forelder', () => {
    ...
    test('Oversikten over første barn, spørsmålflyt for andre barn', async () => {
      ...
      expect(
        screen.queryByRole('group', {
          name: 'Har den andre forelderen samvær med GAMMEL TRUBADUR?',
        })
      ).not.toBeInTheDocument();
      ...
      expect(
        screen.queryByRole('group', {
          name: 'Har dere skriftlig samværsavtale for GAMMEL TRUBADUR?',
        })
      ).not.toBeInTheDocument();
    ...
    test('Oversikten over begge barn', async () => {
      ...
      expect(
        screen.getByRole('heading', { level: 3, name: 'GAMMEL TRUBADUR' })
      ).toBeInTheDocument();
  ...
  describe('Oppsummeringssiden viser riktig informasjon', () => {
    ...
    test('Oversikten viser har ukjent samvær, ikke-beskrivende samværsavtale', async () => {
      ...
      await user.type(
        screen.getByTestId('hvordanPraktiseresSamværet'),
        'En beskrivelse av samværet'
      );
